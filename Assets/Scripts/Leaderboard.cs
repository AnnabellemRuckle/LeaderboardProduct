...
text = (-(float)leaderboard[x].StatValue * 0.001f).ToString("F2");
    ...
    {
        // NOTE: the original version of this game used server-side automation in 
//Javascript to update 
        // the leaderboard. Microsoft removed that feature for new PlayFab 
//applications, so we'll demonstrate 
        // updating a leaderboard using two different methods below. The 'legacy 
//method' calls the custom server-side
        // javascript (cloud script). The alternative uses the latest 
//PlayFabClientAPI to update the player's best score 
